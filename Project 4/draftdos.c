...
int cache_access(int addr, int write_flag, int write_data)
{
    ...
    // if issa store
        ...
        //if issa hit
        if(tag_in_set(tag, set) != 0){
            cache.blocks[tag_in_set(tag, set)].data[offset] = write_data;
            cache.blocks[tag_in_set(tag, set)].dirty = 1;
            // update lru
            for (int p = 0; p < total_num_of_blocks; p++)
            {
                if (cache.blocks[p].set == set)
