...
typedef struct stateStruct {
	...
	MEMWBType MEMWB;
	WBENDType WBEND;
    int fin; // # of finished instructions
    int get; // # of instructions fetched
    int beq_Mispredictions; // # of mispredicted branches
    int beq; // # of braches taken
...
int main(int argc, char *argv[]) {
    ...
    // Initialize instruction fields to the noop instruction
    state.pc = 0;
    state.cycles = 0;
    memset(state.reg, 0, NUMREGS * sizeof(int));
    state.IFID.instr = NOOPINSTRUCTION; // fetch
    state.IDEX.instr = NOOPINSTRUCTION; // decode
    state.EXMEM.instr = NOOPINSTRUCTION; // execute
    state.MEMWB.instr = NOOPINSTRUCTION; // memory
    state.WBEND.instr = NOOPINSTRUCTION; // write back
    ...
    while (opcode(state.MEMWB.instr) != HALT) {
        ...
        if(opcode(newState.IFID.instr == HALT)){
            halt_ = 1;
            newState.get = state.get + 1;
        }
        ...
        else if(halt_ && newState.IFID.instr != 0){
            newState.get = state.get + 1;
        }

        /* ---------------------- ID stage --------------------- */
        currentInstruction = state.IFID.instr;
        
        if(opcode(currentInstruction) == LW && ((field0(currentInstruction) == field0(newState.IFID.instr)) || field0(currentInstruction) == field1(newState.IFID.instr))){
            newState.IFID.instr = NOOPINSTRUCTION;
            newState.IFID.pcPlus1 = 0;
            newState.pc = state.pc;
            subfin--;
            newState.get = state.get;
        }

        offset = convertNum(field2(currentInstruction));
        val_in_regA = state.reg[field0(currentInstruction)];
        val_in_regB = state.reg[field1(currentInstruction)];

        newState.IDEX.pcPlus1 = state.IFID.pcPlus1;
        newState.IDEX.instr = state.IFID.instr;
        newState.IDEX.readRegA = val_in_regA;
        newState.IDEX.readRegB = val_in_regB;
        ...
        /* --------------------- MEM stage --------------------- */
        currentInstruction = state.EXMEM.instr;
        if(opcode)

        /* ---------------------- WB stage --------------------- */
        currentInstruction = state.MEMWB.instr;
        if(opcode(currentInstruction) == ADD){
            ...
            newState.reg[field0(currentInstruction)] = state.MEMWB.writeData;
        }
