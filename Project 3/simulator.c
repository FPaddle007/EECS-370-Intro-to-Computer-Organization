...
int main(int argc, char *argv[]) {
    ...
    while (opcode(state.MEMWB.instr) != HALT) {
        ...
        if(opcode(state.IDEX.instr) == LW){
            ...
            else if((field0(newState.IDEX.instr) == field1(state.IDEX.instr)) || (field1(newState.IDEX.instr) == field1(state.IDEX.instr))){
                    newState.IDEX.instr = NOOPINSTRUCTION;
                    //printf("160");
                    newState.pc = state.pc;
                    newState.IFID = state.IFID;
        ...
        // put dest_MEMWB in field2 (3rd val)
            ...
            dest_MEMWB = field2(state.MEMWB.instr);
        }
        ...
        if(opcode(currentInstruction) == ADD){
            ...
            newState.EXMEM.aluResult = val_in_regA - val_in_regB;
            newState.EXMEM.eq = (val_in_regA == val_in_regB);
        }
